<div>
    <div class=" bg-gray-100 ">
        <div class="bg-white p-6 shadow rounded-md">
            <h2 class="text-2xl font-semibold mb-4">{{ __('Scheduled Appointments') }}</h2>
            <div class="mb-4">
                <label for="search" class="block text-gray-700">{{ __('Enter the name, appointment number') }}</label>
                <div class="flex mt-2">
                    <input id="search" type="text" placeholder="{{ __('Enter the name, appointment number') }}"
                    ...
                    <tbody>
                        @forelse ($appointments as $appointment)
                            <tr wire:key="{{ $appointment->id }}" class="text-gray-700">
                                <td class="border border-gray-300 p-2 text-center">{{ $appointment->id ?? 'NA' }}</td>
