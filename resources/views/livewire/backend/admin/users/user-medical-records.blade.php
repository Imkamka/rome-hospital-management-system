    ...
    <div class=" bg-gray-100 ">
        <div class="bg-white p-6 shadow rounded-md">
            <div class="flex justify-between items-center">
                <h2 class="text-2xl font-semibold mb-4">{{ __('Lab Records') }}</h2>
                <a wire:navigate href="{{ route('admin.users-list') }}"
                    class="bg-blue-500 text-white text-sm font-bold py-2 px-4 rounded transition hover:bg-blue-600">{{ __('Manage Patients') }}</a>
            </div>
            <div class="mb-4">
                ...
                <table class="w-full border-collapse border border-gray-300">
                    <thead>
                        <tr class="bg-gray-200">
                            ...
                            <th class="border border-gray-300 p-2">{{ __('Patient Name') }}</th>
                        ...
                        @forelse ($records as $list)
                            <tr class="text-gray-700" wire:key={{ $list->id }}>
                                ...
                                <td class="border border-gray-300 p-2 text-center">
                                    {{ $list->patient->first_name . ' ' . $list->patient->last_name }}</td>
                                <td class="border border-gray-300 p-2 text-center">
                                    {{ $list->created_at->format('d M,Y') }}</td>
                                <td class="border border-gray-300 p-2 text-center text-blue-600">
                                    <a wire:navigate href="{{ route('record.view', $list->id) }}"
